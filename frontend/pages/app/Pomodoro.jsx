...
export default function Pomodoro() {
  ...
  const [subject, setSubject] = useState(localStorage.getItem('a12_pomo_subject') || 'Physics');
  ...
  const completePhase = () => {
    ...
    if (phase === 'focus') {
      ...
      // Log session with subject + task for heatmap / analytics
      try {
        const log = JSON.parse(localStorage.getItem('a12_pomo_log') || '[]');
        log.push({ ts: Date.now(), subject, task: taskNote, minutes: cfg.focus });
        localStorage.setItem('a12_pomo_log', JSON.stringify(log.slice(-200)));
        localStorage.setItem('a12_pomo_subject', subject);
      } catch {}
      window.__a12Toast && window.__a12Toast(`+${cfg.focus} min on ${subject} logged 🍅`, 'success');
      window.__a12AddXP && window.__a12AddXP(cfg.focus, `Pomodoro · ${subject}`);
        ...
        <Reveal variant="scale">
          <div className="glass-strong" style={{ padding: 36, borderRadius: 24, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {/* Task Linker */}
              <div style={{ display: 'flex', gap: 10, marginTop: 20, marginBottom: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
                <select value={subject} onChange={e => setSubject(e.target.value)}
                  style={{ padding: 10, borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', minWidth: 140 }}>
                  <option>Physics</option><option>Chemistry</option><option>Biology</option><option>Maths</option>
                </select>
                <input value={taskNote} onChange={e => setTaskNote(e.target.value)} placeholder="What are you studying?"
                  style={{ padding: 10, borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', minWidth: 220 }} />
              </div>
