...
export default function RankPredictor() {
  ...
  const [homeState, setHomeState] = useState(user.state || 'All India');
  const [result, setResult] = useState(null);

  const STATES = ['All India','Andhra Pradesh','Assam','Bihar','Delhi','Gujarat','Karnataka','Kerala','Madhya Pradesh','Maharashtra','Odisha','Punjab','Rajasthan','Tamil Nadu','Telangana','Uttar Pradesh','Uttarakhand','West Bengal'];
  ...
  const predict = () => {
    ...
    const eligible = colleges
      ...
      .map(c => {
        const baseCutoff = c.cutoffs[category] || c.cutoffs.General;
        const stateBonus = homeState !== 'All India' && c.state === homeState ? 1.35 : 1.0;
        return { ...c, effectiveCutoff: Math.round(baseCutoff * stateBonus), baseCutoff, isHomeState: stateBonus > 1 };
      })
      .filter(c => pred.estimatedRank <= c.effectiveCutoff * 1.5)
      .sort((a, b) => a.nirf - b.nirf)
      .slice(0, 12);
    setResult({ ...pred, eligible, score: s, category, homeState });
  };
      ...
      <Reveal variant="up" delay={100}>
        <div className="glass" style={{ padding: 28, marginBottom: 24, maxWidth: 600, borderRadius: 18 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: 13, color: 'var(--text-mid)', marginBottom: 8 }}>Home State</label>
              <select value={homeState} onChange={e => { setHomeState(e.target.value); setResult(null); }} style={inputStyle}>
                {STATES.map(s => <option key={s} style={{ background: '#0F1530' }}>{s}</option>)}
              </select>
            </div>
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: 16 }}>
                {result.eligible.map((c, i) => {
                  const cutoff = c.effectiveCutoff;
                    ...
                    <Reveal key={c.id} variant="up" delay={i * 70}>
                      <div className="glass" style={{ padding: 22, borderRadius: 14, transition: 'all 0.3s' }}
                        ...
                        <div style={{ fontSize: 13, color: 'var(--text-dim)', marginBottom: 4 }}>
                          📍 {c.city}, {c.state}
                          {c.isHomeState && <span style={{ marginLeft: 6, fontSize: 10, padding: '2px 8px', borderRadius: 999, background: 'rgba(16,185,129,0.18)', color: 'var(--green)', fontWeight: 700 }}>HOME STATE</span>}
                        </div>
                        <div style={{ fontSize: 13, color: 'var(--text-dim)', marginBottom: 8 }}>NIRF #{c.nirf} · ₹{(c.fees / 100000).toFixed(1)}L/year</div>
                        <div style={{ fontSize: 12, color: 'var(--text-mid)' }}>
                          2024 Cutoff ({result.category}): <strong style={{ color: 'var(--violet-2)' }}>{c.baseCutoff}</strong>
                          {c.isHomeState && <span style={{ color: 'var(--green)', marginLeft: 4 }}> → {c.effectiveCutoff} (state quota)</span>}
                        </div>
                      </div>
                    </Reveal>
                  );
